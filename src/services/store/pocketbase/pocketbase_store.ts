import { file_type } from '../../../models/transcript';
...
export const pbFileStore: FileStore = {
     ...
     */
    async uploadFile(file: File, type: file_type): Promise<{ file_id: string; file_url: string; }> {
        const data = {
            file_type: type,
            file: file,
        }
        ...
        try {
            const record = await pb.collection(FILE_COLLECTION).create<FileRecord>(data);
