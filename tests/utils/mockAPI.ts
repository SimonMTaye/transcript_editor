...
export const mockApiFactory = (transcripts: Transcript[] = []) => ({
  getRecentTranscripts: vi.fn(async (page: number = 1) => {
    const metas = transcripts.map<TranscriptMeta>(
      (t) => splitTranscriptIntoMetaAndData(t).meta
    );
    return metas
      .filter((t) => t.status === "ready")
      .slice(
        (page - 1) * TRANSCRIPTS_SUMMARIES_LIMIT,
        page * TRANSCRIPTS_SUMMARIES_LIMIT
      );
  }),
  ...
  saveTranscriptEdits: vi.fn(
    async (id: string, segments: TranscriptSegment[]) => {
      const transcript = transcripts.find((t) => t.id === id);
      if (!transcript) {
        throw new Error(`Transcript with ID ${id} not found`);
      }
...
export const createDummyTranscripts = (
  count: number = 20,
  numSegments: number = 1
): Transcript[] => {
  const dummySegments = Array.from({ length: numSegments }, (_, i) => ({
    start: i * 10,
    end: (i + 1) * 10,
    text: `Dummy segment ${i + 1} text.`,
    speaker: i % 2 === 0 ? "SPEAKER_01" : "SPEAKER_02",
  }));
...
export const mockTranscriptApi: typeof transcriptApi = mockApiFactory(
  createDummyTranscripts()
);
